...
#output_name = '/rs1/researchers/y/yliu335/ddong2/gpu-runs/data/test'
...
def initialize_circuit():
    ...
    cdp = ClassicalRegister(size = numberofmodes)
    circuit = c2qa.CVCircuit(qmr, qbr, qdp, cdp)
    return qmr, qbr, qdp, cdp, circuit
...
# Damping phase so damping probability is sin(theta/2)^2 = (gamma*timestep)
gamma = np.array([3.15e12/1e12, 1.05e12/1e12, 9.45e12/1e12, 3.15e12/1e12]) # [gamma_a, gamma_b, gamma_c, gamma_l]
...
for i in tqdm(range(int(sim_time/timestep))):
    ...
    qmr, qbr, qdp, cdp, circuit = initialize_circuit()
    circuit.x(qbr[0])
    ...
    # Now include damping from https://arxiv.org/pdf/2302.14592.pdf (Fig 8)
        for idx_qb in range(numberofmodes):
            ...
            circuit.measure(qdp[idx_qb], cdp[idx_qb])
