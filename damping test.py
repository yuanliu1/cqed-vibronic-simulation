...
# Data values
gamma = np.array([1.15e13/1e12, 1.45e13/1e12])
gamma = gamma
...
for current_time in time:
    ...
    # Initialize Circuit
    qumode_reg = c2qa.QumodeRegister(num_qumodes=2, num_qubits_per_qumode=1)
    ...
    for trotter_step in range(trotter_steps):
        ...
        '''
       # for bit_idx in range(2):
       #     circuit.ry(-theta_damp[bit_idx], ancilla_reg[bit_idx])
       #     circuit.z(ancilla_reg[bit_idx])
       #     circuit.cz(data_reg[bit_idx], ancilla_reg[bit_idx])
    ...
    rho_A.append(np.asarray(partial_trace(density_matrix, [1,2,3]))[1,1])
    rho_B.append(np.asarray(partial_trace(density_matrix, [0,2,3]))[1,1])
    qumode_matrix = DensityMatrix(np.asarray(c2qa.util.trace_out_qubits(circuit, stateop)))
    qumode_A.append(np.asarray(partial_trace(density_matrix, [1]))[1,1])
    qumode_B.append(np.asarray(partial_trace(density_matrix, [0]))[1,1])
